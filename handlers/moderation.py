...
def setup_moderation(dp: Dispatcher):
    ...
    async def cmd_mute(message: Message):
        ...
        if await is_admin(message.chat, user_id):
            ...
            return
        await message.chat.restrict(user_id, permissions=ChatPermissions(can_send_messages=False), until_date=seconds)
    ...
    async def cmd_unmute(message: Message):
        ...
        if not target:
            ...
            return
        user_id, _ = target
        await message.chat.restrict(user_id, permissions=ChatPermissions(can_send_messages=True))
